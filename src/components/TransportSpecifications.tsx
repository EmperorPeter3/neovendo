...
export const TransportSpecifications = ({ listing }: TransportSpecificationsProps) => {
  ...
  function buildCarSpecs(s: SpecItem[]) {
    ...
    if (l.car_mileage != null) addSpec(<Gauge className="w-5 h-5" />, tAny('carFilters.mileage'), `${l.car_mileage.toLocaleString()} km`);
    if (l.car_body_type) {
    ...
    if (l.car_engine_volume) addSpec(<Wrench className="w-5 h-5" />, tAny('carFilters.engineVolume'), `${l.car_engine_volume} cc`);
    if (l.car_power) addSpec(<Zap className="w-5 h-5" />, tAny('carFilters.power'), `${l.car_power} hp`);
