...
export interface SnowmobileFieldsData {
  type: string;
  brand: string;
  model: string;
...
export const defaultSnowmobileFields: SnowmobileFieldsData = {
  type: '',
  brand: '',
  model: '',
  originCountry: '',
...
interface SnowmobileFieldsFormProps {
  data: SnowmobileFieldsData;
  onChange: (data: SnowmobileFieldsData) => void;
...
export function SnowmobileFieldsForm({ data, onChange, fieldErrors = {}, onClearError }: SnowmobileFieldsFormProps) {
  const { t } = useLanguage();
  const yearOptions = useMemo(() => getYearOptions(), []);

  const handleChange = <K extends keyof SnowmobileFieldsData>(key: K, value: SnowmobileFieldsData[K]) => {
  ...
  return (
    <div className="space-y-6 border-t pt-6 mt-6">
      <h3 className="font-semibold text-foreground">{safeT(t, 'snowmobileFilters.title')}</h3>
      ...
      <div className="space-y-2">
        ...
        <RadioGroup
          ...
          onValueChange={(value) => handleChange('condition', value as SnowmobileFieldsData['condition'])}
          className="flex flex-wrap gap-4"
        >
          {snowmobileConditions.map(condition => (
            <div key={condition.id} className="flex items-center space-x-2">
              <RadioGroupItem value={condition.id} id={`snow-form-condition-${condition.id}`} />
              <Label htmlFor={`snow-form-condition-${condition.id}`} className="font-normal cursor-pointer">
      ...
      {data.condition !== 'new' && (
        <div className="space-y-2">
          <Label className="text-sm font-medium">
            {safeT(t, 'snowmobileFilters.mileage')}
      ...
      <div className="space-y-2">
        ...
        <Select value={data.engineType} onValueChange={(v) => handleChange('engineType', v)}>
          ...
          <SelectContent>
            {snowmobileEngineTypes.map(type => (
              <SelectItem key={type.id} value={type.id}>{safeT(t, type.translationKey)}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      ...
      <div className="space-y-2">
        <Label className="text-sm font-medium">
          {safeT(t, 'snowmobileFilters.engineVolume')}
      ...
      {/* Max Passengers */}
      <div className="space-y-2">
        <Label className="text-sm font-medium">{safeT(t, 'snowmobileFilters.maxPassengers')}</Label>
        <Input
          type="number"
          placeholder="1"
      ...
      <div className="space-y-2">
        <Label className="text-sm font-medium">
          {safeT(t, 'snowmobileFilters.trackWidth')}
          <span className="text-muted-foreground ml-1">({safeT(t, 'units.mm')})</span>
        </Label>
        <Input
          type="number"
          placeholder="0"
          value={data.trackWidth}
          onChange={(e) => handleChange('trackWidth', e.target.value)}
          min="0"
          className={getInputClass('trackWidth')}
        />
      </div>
    </div>
  );
